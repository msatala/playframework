...
object PlayDocsValidation extends PlayDocsValidationCompat {
  ...
  val validateDocsTask = Def.task {
    ...
    assertLinksNotMissing(
      ...
      report.wikiLinks.filterNot { link =>
        pages.contains(link.link) || validationConfig
          .downstreamWikiPages(link.link) || combinedRepo.findFileWithName(link.link + ".md").nonEmpty
      },
      "Could not find link"
    )
    ...
    def segmentExists(sample: CodeSampleRef) = {
      if (sample.segment.nonEmpty) {
        ...
        val sourceCode =
          combinedRepo.loadFile(sample.source)(is => IO.readLines(new BufferedReader(new InputStreamReader(is)))).get
  ...
  val validateExternalLinksTask = Def.task {
    ...
    val grouped = report.externalLinks
      .groupBy { _.link }
      .filterNot { e =>
        e._1.startsWith("http://localhost:") || e._1.contains("example.com") || e._1.startsWith("http://127.0.0.1")
      }
      .toSeq
      .sortBy { _._1 }
