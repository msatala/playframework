...
import shaded26.play.routes.compiler.RoutesCompiler.RoutesCompilerTask
import play.TemplateImports
import shaded26.play.sbt.routes.RoutesKeys26._
import sbt._
import sbt.Keys._
...
object Imports {
  object PlayDocsKeys {
    ...
    val docsJarScalaBinaryVersion =
      SettingKey[String]("playDocsScalaVersion", "The binary scala version of the documentation", KeyRanks.BSetting)
 ...
 */
object PlayDocsPlugin26 extends AutoPlugin with PlayDocsPluginCompat {
  ...
  override def requires = RoutesCompiler26
  ...
  def docsTestSettings = Seq(
    ...
    routesGenerator26 := InjectedRoutesGenerator,
    evaluateSbtFiles := {
      ...
      val result = sbtFiles.map { sbtFile =>
        ...
        try {
          ...
          log.info(s"  ${Colors26.green("+")} $relativeFile")
          true
        } catch {
          case NonFatal(_) =>
            log.error(s" ${Colors26.yellow("x")} $relativeFile")
            false
        }
      }
  ...
  val docsRunSetting: Def.Initialize[InputTask[Unit]] = Def.inputTask {
    ...
    println()
    println(
      Colors26.green("Documentation server started, you can now view the docs by going to http://" + server.mainAddress())
    )
    println()
