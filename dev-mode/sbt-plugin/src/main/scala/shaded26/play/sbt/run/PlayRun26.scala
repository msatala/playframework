...
import shaded26.play.sbt._
import shaded26.play.sbt.PlayImport26._
 ...
 */
object PlayRun26 extends PlayRunCompat {
  ...
  val playDefaultRunTask =
    playRunTask(playRunHooks26, playDependencyClasspath26, playReloaderClasspath26, playAssetsClassLoader26)
   ...
   */
  def playRunTask(
                   runHooks: TaskKey[Seq[PlayRunHook26]],
    ...
    val interaction = playInteractionMode26.value

    val reloadCompile = () =>
      PlayReload26.compile(
        () => Project.runTask(playReload26 in scope, state).map(_._2).get,
    ...
    lazy val devModeServer = Reloader.startDevMode(
      ...
      playCommonClassloader26.value,
      dependencyClasspath.value.files,
      reloadCompile,
      assetsClassLoader.value,
      playMonitoredFiles26.value,
      fileWatchService26.value,
      generatedSourceHandlers,
      playDefaultPort26.value,
      playDefaultAddress26.value,
      baseDirectory.value,
      devSettings26.value,
      args,
    ...
    interaction match {
      ...
      case blocking =>
        ...
        println()
        println(Colors26.green("(Server started, use Enter to stop and go back to the console...)"))
        println()
    ...
    if (triggered) {
      ...
      Project.synchronized {
        ...
        Project.runTask(compile in Compile, newState).get._2.toEither.right.map { _ =>
          ...
          println("[" + Colors26.green("success") + "] Compiled in " + formatted)
        }
      }
  ...
  val playPrefixAndAssetsSetting = playPrefixAndAssets26 := {
    assetsPrefix26.value -> (WebKeys.public in Assets).value
  }

  val playAllAssetsSetting = playAllAssets26 := Seq(playPrefixAndAssets26.value)

  val playAssetsClassLoaderSetting = playAssetsClassLoader26 := {
    val playAllAssetsValue = playAllAssets26.value
  ...
  private def testProd(state: State, args: Seq[String]): State = {
    ...
    val interaction = extracted.get(playInteractionMode26)
    ...
    val (properties, httpPort, httpsPort, httpAddress) =
      Reloader.filterArgs(filtered, extracted.get(playDefaultPort26), extracted.get(playDefaultAddress26), devSettings)
