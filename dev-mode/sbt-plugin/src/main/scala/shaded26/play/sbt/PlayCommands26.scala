...
import PlayInternalKeys26._
import sbt.Keys._
import sbt._

object PlayCommands26 {

  val playReloadTask = Def.task {
    playCompileEverything26.value.reduceLeft(_ ++ _)
  }
  ...
  val playCompileEverythingTask = Def.taskDyn {
    ...
    val compileTask = Def.taskDyn(playAssetsWithCompilation26 ?? (compile in Compile).value)
  ...
  val h2Command = Command.command("h2-browser") { state: State =>
    try {
      val commonLoader  = Project.runTask(playCommonClassloader26, state).get._2.toEither.right.get
