...
import shaded26.play.sbt.PlayImport26.PlayKeys26._
import shaded26.play.sbt.PlayInternalKeys26._
import shaded26.play.sbt.routes.RoutesKeys26
import shaded26.play.sbt.run._
import shaded26.play.sbt.run.PlayRun26.DocsApplication
...
object PlaySettings26 extends PlaySettingsCompat26 {
  ...
  lazy val serviceSettings = Seq[Setting[_]](
    playPlugin26 := false,
    externalizeResources26 := true,
    includeDocumentationInBinary26 := true,
    javacOptions in (Compile, doc) := List("-encoding", "utf8"),
    libraryDependencies += {
      if (playPlugin26.value) {
    ...
    playOmnidoc26 := !play.core.PlayVersion.current.endsWith("-SNAPSHOT"),
    playDocsName26 := { if (playOmnidoc26.value) "play-omnidoc" else "play-docs" },
    playDocsModule26 := Some(
      "com.typesafe.play" %% playDocsName26.value % play.core.PlayVersion.current % DocsApplication.name
    ),
    libraryDependencies ++= playDocsModule26.value.toSeq,
    manageClasspath(DocsApplication),
    playDocsJar26 := (managedClasspath in DocsApplication).value.files.find(_.getName.startsWith(playDocsName26.value)),
    ...
    PlayInternalKeys26.playAllAssets26 := Seq.empty,
    PlayRun26.playAssetsClassLoaderSetting,
    // THE `in Compile` IS IMPORTANT!
    Keys.run in Compile := PlayRun26.playDefaultRunTask.evaluated,
    mainClass in (Compile, Keys.run) := Some("play.core.server.DevServerStart"),
    PlayInternalKeys26.playStop26 := {
      playInteractionMode26.value match {
    ...
    shellPrompt := PlayCommands26.playPrompt,
    // all dependencies from outside the project (all dependency jars)
    playDependencyClasspath26 := (externalDependencyClasspath in Runtime).value,
    // all user classes, in this project and any other subprojects that it depends on
    playReloaderClasspath26 := Classpaths
    ...
    playReloaderClasspath26 ~= { _.filter(_.get(WebKeys.webModulesLib.key).isEmpty) },
    playCommonClassloader26 := PlayCommands26.playCommonClassloaderTask.value,
    playCompileEverything26 := getPlayCompileEverything(PlayCommands26.playCompileEverythingTask.value),
    playReload26 := PlayCommands26.playReloadTask.value,
    ivyLoggingLevel := UpdateLogging.DownloadOnly,
    playMonitoredFiles26 := PlayCommands26.playMonitoredFilesTask.value,
    fileWatchService26 := FileWatchService
      .defaultWatchService(target.value, getPoolInterval(pollInterval.value).toMillis.toInt, sLog.value),
    playDefaultPort26 := 9000,
    playDefaultAddress26 := "0.0.0.0",
    // Default hooks
    playRunHooks26 := Nil,
    playInteractionMode26 := PlayConsoleInteractionMode,
    // Settings
    devSettings26 := Nil,
    // Native packaging
    ...
    mappings in Universal ++= {
      val resourceMappings = (playExternalizedResources26 in Compile).value
      if (externalizeResources26.value) {
    ...
    scriptClasspathOrdering := Def.taskDyn {
      ...
      if (externalizeResources26.value) {
        Def.task {
          ...
          val jarSansExternalized = (playJarSansExternalized26 in Runtime).value
          oldValue.map {
            case (packageBinJar, _) if jar == packageBinJar =>
              val id  = projectID.value
              val art = (artifact in Compile in playJarSansExternalized26).value
              val jarName =
    ...
    mappings in Universal ++= Def.taskDyn {
      ...
      if (includeDocumentationInBinary26.value) {
        Def.task {
    ...
    generateSecret26 := ApplicationSecretGenerator26.generateSecretTask.value,
    updateSecret26 := ApplicationSecretGenerator26.updateSecretTask.value,
    // by default, compile any routes files in the root named "routes" or "*.routes"
    sources in (Compile, RoutesKeys26.routes26) ++= {
  ...
  lazy val webSettings = Seq[Setting[_]](
    ...
    RoutesKeys26.routesImport26 ++= Seq("controllers.Assets.Asset"),
    // sbt-web
    ...
    // Assets for run mode
    PlayRun26.playPrefixAndAssetsSetting,
    PlayRun26.playAllAssetsSetting,
    assetsPrefix26 := "public/",
    // Assets for distribution
    WebKeys.packagePrefix in Assets := assetsPrefix26.value,
    playPackageAssets26 := (packageBin in Assets).value,
    scriptClasspathOrdering += {
      ...
      playPackageAssets26.value -> ("lib/" + jarName)
    },
    // Assets for testing
    public in TestAssets := (public in TestAssets).value / assetsPrefix26.value,
  ...
  private def externalizedSettings: Seq[Setting[_]] =
    Defaults.packageTaskSettings(playJarSansExternalized26, mappings in playJarSansExternalized26) ++ Seq(
      playExternalizedResources26 := getPlayExternalizedResources(
        unmanagedResourceDirectories.value,
        unmanagedResources.value
      ),
      mappings in playJarSansExternalized26 := {
        ...
        val externalized       = playExternalizedResources26.value.map(_._1).toSet
      ...
      artifactClassifier in playJarSansExternalized26 := Option("sans-externalized")
    )
